...
module.exports = class SteamManifestComparison {
  ...
  manifestFiles (manifest, directory) {
    return manifest.files
      ...
      .map(file => (
        {
          ...file,
          filename: this.resolvePath(directory, file.filename.toLowerCase())
        }
      ))
      .filter(file => {
        return !this.containsIgnoredManifestPath(file.filename)
      })
  }
  ...
  async extraLocalFiles (manifestFiles, directory) {
    const manifestFilenames = manifestFiles.map(file => file.filename)
